...
G4ReactionProduct * ParticleHPEnAngCorrelation::SampleOne(G4double anEnergy)
{ 
...
G4ReactionProductVector * ParticleHPEnAngCorrelation::Sample(G4double anEnergy)
{
  ...
  for(i=0; i<nProducts; i++)
  {
	...
	if(frameFlag==1) // target rest //TK 100413 should be LAB?
	{
	  ...
	  if( std::getenv("ParticleHPDebug") ) 
	    G4cout <<"ParticleHPEnAngCorrelation: before Lorentz boost "<<
...
#ifdef G4PHPDEBUG
	  if( std::getenv("ParticleHPDebug") ) 
	    G4cout <<"ParticleHPEnAngCorrelation: after Lorentz boost "<<
		it->at(ii)->GetKineticEnergy()<<" "<<
	      it->at(ii)->GetMomentum()<<G4endl;
#endif
	}
	...
	{
	  throw G4HadronicException(__FILE__, __LINE__, "ParticleHPEnAngCorrelation::Sample: The frame of the finalstate is not specified");
	}
