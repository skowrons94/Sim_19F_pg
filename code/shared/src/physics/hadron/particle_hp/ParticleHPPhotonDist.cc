...
G4bool ParticleHPPhotonDist::InitMean(std::istream & aDataFile)
{
  ...
  if(aDataFile >> repFlag)
  {
    ...
    // multiplicities
      ...
      theYield = new ParticleHPVector[nDiscrete];
    ...
    else if(repFlag == 2)
    {
	 ...
	 {
           throw G4HadronicException(__FILE__, __LINE__, "ParticleHPPhotonDist: Unknown conversion flag");
	 }
      }
       // Note, that this is equivalent to using the 'Gamma' classes.
      // throw G4HadronicException(__FILE__, __LINE__, "ParticleHPPhotonDist: Transition probability array not sampled for the moment.");
    }
    else
    {
      G4cout << "Data representation in ParticleHPPhotonDist: "<<repFlag<<G4endl;
      throw G4HadronicException(__FILE__, __LINE__, "ParticleHPPhotonDist: This data representation is not implemented.");
    }
  }
  else
  {
...
void ParticleHPPhotonDist::InitAngular(std::istream & aDataFile)
{
  G4int i, ii;
  ...
  if (isoFlag != 1)
  {
    if (repFlag == 2) G4cout << "ParticleHPPhotonDist: repFlag == 2 && isoFlag != 1 is unexpected! If you use G4ND3.x, then please report to Geant4 HyperNews. " << G4endl;
    aDataFile >> tabulationType >> nDiscrete2 >> nIso;
//080731
      if (theGammas != NULL && nDiscrete2 != nDiscrete) 
        G4cout << "080731c ParticleHPPhotonDist nDiscrete2 != nDiscrete, It looks like something wrong in your NDL files. Please update the latest. If you still have this messages after the update, then please report to Geant4 Hyper News." << G4endl;
      ...
      std::vector < ParticleHPVector* > vct_pXS_par;
      if ( theGammas != NULL && theShells != NULL ) 
      {
         ...
         for ( i = 0 ; i < nDiscrete ; i++ )
         {
            ...
            ParticleHPVector* hpv = new ParticleHPVector;
...
//080731
    ...
    if(tabulationType==1)theLegendre=new ParticleHPLegendreTable *[nDiscrete2-nIso];
    if(tabulationType==2)theAngular =new ParticleHPAngularP *[nDiscrete2-nIso];
    for(i=nIso; i< nDiscrete2; i++)
    {
      if(tabulationType==1) 
      {
        ...
        theGammas[i]*=eV;
        theShells[i]*=eV;
        theLegendre[i-nIso]=new ParticleHPLegendreTable[nNeu[i-nIso]];
      ...
      else if(tabulationType==2)
      {
        ...
        theGammas[i]*=eV;
        theShells[i]*=eV;
        theAngular[i-nIso]=new ParticleHPAngularP[nNeu[i-nIso]];
        for (ii=0; ii<nNeu[i-nIso]; ii++)
        {
...
//080731
     if ( vct_gammas_par.size() > 0 ) 
     {
        ...
        //Garbage collection 
        for ( std::vector < ParticleHPVector* >::iterator 
...
void ParticleHPPhotonDist::InitEnergies(std::istream & aDataFile)
{
  ...
  probs = new ParticleHPVector[nPartials];
  partials = new ParticleHPPartial * [nPartials];
  G4int nen;
  G4int dummy;
  for (i=0; i<nPartials; i++)
  {
    ...
    aDataFile >> nen;
    partials[i] = new ParticleHPPartial(nen);
    partials[i]->InitInterpolation(aDataFile);
    partials[i]->Init(aDataFile);
  }
}

void ParticleHPPhotonDist::InitPartials(std::istream& aDataFile,
                                          ParticleHPVector* theXsec)
{
  ...
  disType = new G4int[nDiscrete];
  thePartialXsec = new ParticleHPVector[nDiscrete];
  for(i=0; i<nDiscrete; i++)
  {
  ...
  //G4cout << "ParticleHPPhotonDist::InitPartials Test " << G4endl;
  //G4cout << "ParticleHPPhotonDist::InitPartials nDiscrete " << nDiscrete << G4endl;
  //ParticleHPVector* aHP = new ParticleHPVector;
  //aHP->Check(1);
}

G4ReactionProductVector * ParticleHPPhotonDist::GetPhotons(G4double anEnergy)
{

  //G4cout << "ParticleHPPhotonDist::GetPhotons repFlag " << repFlag << G4endl;
  ...
  if (repFlag==1) {
    ...
    if (nDiscrete == 1 && nPartials == 1) {
      if (actualMult.Get()->at(0) > 0) {
        ...
        count++;
	if (count > nSecondaries) throw G4HadronicException(__FILE__, __LINE__, "ParticleHPPhotonDist::GetPhotons inconsistency");
      }
  ...
  //      G4cout << "Isotropic distribution in PhotonDist"<<temp<<G4endl;
	...
	else if(tabulationType==1)
	{
	    ...
	    if(theLegendre[ii-nIso][iii].GetEnergy()>anEnergy)
          ...
          ParticleHPLegendreStore aStore(2);
