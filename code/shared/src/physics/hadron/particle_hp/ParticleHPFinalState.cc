...
void ParticleHPFinalState::adjust_final_state ( G4LorentzVector init_4p_lab )
{

   G4double minimum_energy = 1*keV;

   if ( ParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) return;
   ...
   {
      //G4cout << "ParticleHPFinalState::adjust_final_state theResult.Get()->GetSecondary( i )->GetParticle()->GetDefinition()->GetParticleName() = " << theResult.Get()->GetSecondary( i )->GetParticle()->GetDefinition()->GetParticleName() << G4endl;
...
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))    G4cout << "ParticleHPFinalState::adjust_final_stat SECO " << i << " " <<theResult.Get()->GetSecondary( i )->GetParticle()->GetDefinition()->GetParticleName() << G4endl;
#endif
...
#ifdef G4PHPDEBUG
   if( std::getenv("ParticleHPDebug")) G4cout  << "ParticleHPFinalState::adjust_final_stat  BaseZ " << baseZNew << " BaseA " << baseANew << " sum_Z " << sum_Z << " sum_A " << sum_A << G4endl;
#endif
   ...
   else
   {
	...
	resi_pd = G4IonTable::GetIonTable()->GetIon ( int(baseZNew - sum_Z) , (int)(baseANew - sum_A) , 0.0 );
      ...
      if ( needOneMoreSec )
      {
         ...
         {
#ifdef G4PHPDEBUG
	   if( std::getenv("ParticleHPDebug")) G4cout << this << "ParticleHPFinalState oneMoreSec_pd Z " << baseZNew << " - " << sum_Z << " A " << baseANew << " - " << sum_A << " projectile " << theProjectile->GetParticleName() << G4endl;
#endif
	   oneMoreSec_pd = G4IonTable::GetIonTable()->GetIon ( int(baseZNew - sum_Z) , (int)(baseANew - sum_A) , 0.0 );
	   if( !oneMoreSec_pd ) {
	     G4cerr << this << "ParticleHPFinalState oneMoreSec_pd Z " << baseZNew << " - " << sum_Z << " A " << baseANew << " - " << sum_A << " projectile " << theProjectile->GetParticleName() << G4endl;
	     G4Exception("ParticleHPFinalState:adjust_final_state",
			 "Warning",
			 JustWarning,
	...
	if( theProjectile == G4Neutron::Neutron() ) {
	  ...
	  ndlANew += 12;
	   ...
	   if( !resi_pd ) {
	     G4cerr << "ParticleHPFinalState resi_pd Z " << max_SecZ << " - " << dif_Z << " A " << max_SecA << " - " << dif_A << " projectile " << theProjectile->GetParticleName() << G4endl;
	     G4Exception("ParticleHPFinalState:adjust_final_state",
			 "Warning",
			 JustWarning,
			 "No adjustment will be done!");
