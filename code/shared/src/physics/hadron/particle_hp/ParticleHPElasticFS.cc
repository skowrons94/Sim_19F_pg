...
//
#include "ParticleHPElasticFS.hh"
...
ParticleHPElasticFS::ParticleHPElasticFS()
{
...
void ParticleHPElasticFS::Init(G4double A, G4double Z, G4int M,
                                 G4String& dirName, G4String&,
                                 G4ParticleDefinition* )
{
  G4String tString = "/FS";
  G4bool dbool;
  ParticleHPDataUsed aFile =
  ...
  if (repFlag == 1) {
    G4int nEnergy;
    theData >> nEnergy; 
    theCoefficients = new ParticleHPLegendreStore(nEnergy);
    ...
    G4int nEnergy;
    theData >> nEnergy;
    theProbArray = new ParticleHPPartial(nEnergy, nEnergy);
    ...
    if (nEnergy_Legendre <= 0 ) {
      ...
      iss << "ParticleHPElasticFS::Init Data Error repFlag is 3 but nEnergy_Legendre <= 0";
    ...
    theCoefficients = new ParticleHPLegendreStore( nEnergy_Legendre );
    theCoefficients->InitInterpolation( theData );
    G4double temp, energy;
    G4int tempdep, nLegendre;
    ...
    theProbArray = new ParticleHPPartial( nEnergy_Prob , nEnergy_Prob );
    theProbArray->InitInterpolation( theData );
    G4int nPoints;
    for (G4int i = 0; i < nEnergy_Prob; i++) {
      ...
      throw G4HadronicException(__FILE__, __LINE__, "ParticleHPElasticFS::Init -- unusable number for repFlag");
  }
...
G4HadFinalState*
ParticleHPElasticFS::ApplyYourself(const G4HadProjectile& theTrack)
{  
  ...
  if (repFlag == 1) {
    ...
    throw G4HadronicException(__FILE__, __LINE__,
                              "ParticleHPElasticFS::Init -- unusable number for repFlag");
  }
  ...
  if (frameFlag == 1) {
    ...
    throw G4HadronicException(__FILE__, __LINE__,
                   "ParticleHPElasticFS::ApplyYourSelf frameflag incorrect");
  }
