...
G4ThreadLocal G4int ParticleHPChannelList::trycounter = 0;

 ParticleHPChannelList::ParticleHPChannelList(G4int n, G4ParticleDefinition* projectile)
  :theProjectile(projectile)
  { 
    nChannels = n;
    theChannels  = new ParticleHPChannel * [n];
...
#include "G4Neutron.hh"
  ParticleHPChannelList::ParticleHPChannelList()
  {
    nChannels = 0;
    theChannels = 0;
    allChannelsCreated = false;
  ...
  G4HadFinalState * ParticleHPChannelList::ApplyYourself(const G4Element * , const G4HadProjectile & aTrack)
  {
    ...
    for(i=0; i<nChannels; i++)
    {
      ...
      if(theChannels[i]->HasAnyData(isotope))
      {
        ...
        targA=(G4int)theChannels[i]->GetN(isotope); //Will be simply used the last valid value
        targZ=(G4int)theChannels[i]->GetZ(isotope);
	//	G4cout << " ParticleHPChannelList " << i << " targA " << targA << " targZ " << targZ << " running " << running[i] << G4endl;
      }
    }
    ...
    if ( running[nChannels-1] == 0 )
    {
       ...
       //TK121106
       ParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargA( targA ); 
       ParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargZ( targZ ); 
       delete [] running;
       return &unChanged;
    }
    //TK120607
    ...
    delete [] running;
#ifdef G4PHPDEBUG
    if( std::getenv("ParticleHPDebug") ) G4cout << " ParticleHPChannelList SELECTED ISOTOPE " << isotope << " SELECTED CHANNEL " << lChan << G4endl;
#endif
    return theChannels[lChan]->ApplyYourself(aTrack, isotope);
  }
      
void ParticleHPChannelList::Init(G4Element * anElement, const G4String & dirName, G4ParticleDefinition* projectile )
  {
    theDir = dirName;
...
//    G4cout << theElement << G4endl;
  ...
  void ParticleHPChannelList::Register(ParticleHPFinalState * theFS, 
					 const G4String & aName )
  {
    if(!allChannelsCreated)
    {
      if(nChannels!=0)
      {
	ParticleHPChannel ** theBuffer = new ParticleHPChannel * [nChannels+1];
	G4int i;
	for(i=0; i<nChannels; i++)
	{
	...
	theChannels = theBuffer;
      ...
      {
	theChannels = new ParticleHPChannel * [nChannels+1];
      }
      G4String name;
      name = aName+"/";
      theChannels[nChannels] = new ParticleHPChannel(theProjectile);
...
void ParticleHPChannelList::DumpInfo(){
