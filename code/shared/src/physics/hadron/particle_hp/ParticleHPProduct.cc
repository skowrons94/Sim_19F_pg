...
//
#include "ParticleHPProduct.hh" 
#include "G4Poisson.hh"
#include "G4Proton.hh"
#include "G4HadronicParameters.hh"

G4int ParticleHPProduct::GetMultiplicity(G4double anEnergy )
{
  ...
  //g  G4cout << "ParticleHPProduct MEAN NUMBER OF PARTICLES " << mean << " for " << theMass << G4endl;
  //if( mean <= 0. ) return 0;
  //151120 TK Modified for solving reproducibility problem 
  ...
  { 
	...
	//       if( CLHEP::RandFlat::shoot() > mean-multi ) multi++;
     ...
     if( std::getenv("ParticleHPDebug") && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
       G4cout << "ParticleHPProduct::GetMultiplicity " << theMassCode << " " << theMass << " multi " << multi << " mean " << mean << G4endl;
     #endif
#endif
  }
...
G4ReactionProductVector * ParticleHPProduct::Sample(G4double anEnergy, G4int multi)
{
  if(theDist == 0) { return 0; }
...
//                  - theActualStateQValue;
 ...
 if( std::getenv("G4PHPTEST") )
    #ifdef G4VERBOSE
    if( std::getenv("ParticleHPDebug") && tmp != 0 && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
      G4cout << multi << " " << i << " @@@ ParticleHPProduct::Sample " << anEnergy << " Mass " << theMassCode << " " << theMass << G4endl;
    #endif
#endif
    ...
    #ifdef G4VERBOSE
    if( std::getenv("ParticleHPDebug") && tmp != 0 && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
      G4cout << multi << " " << i << " @@@ ParticleHPProduct::Sample " << tmp->GetDefinition()->GetParticleName() << " E= " << tmp->GetKineticEnergy() << G4endl;
    #endif
#endif
  }
  if(multi == 0) 
