...
#include "ParticleHPContAngularPar.hh"
...
#include "ParticleHPKallbachMannSyst.hh"
#include "G4IonTable.hh"
#include "ParticleHPManager.hh"
#include <set>
 
ParticleHPContAngularPar::ParticleHPContAngularPar( G4ParticleDefinition* projectile )
{  
  theAngular = 0;
  ...
  adjustResult = true;
  if ( ParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) adjustResult = false;
  ...
  void ParticleHPContAngularPar::Init(std::istream & aDataFile, G4ParticleDefinition* projectile)
  { 
    adjustResult = true;
    if ( ParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) adjustResult = false;
    ...
    theEnergy *= eV;
    theAngular = new ParticleHPList [nEnergies];
  ...
  G4ReactionProduct * 
  ParticleHPContAngularPar::Sample(G4double anEnergy, G4double massCode, G4double /*targetMass*/, 
                                    G4int angularRep, G4int /*interpolE*/ )
  {
    // The following line is needed because it may change between runs by UI command
    if ( ParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) adjustResult = false;
 
    if( std::getenv("G4PHPTEST") ) G4cout << "  ParticleHPContAngularPar::Sample " << anEnergy << " " << massCode << " " << angularRep << G4endl; //GDEB
    if ( fCache.Get() == 0 ) cacheInit();
    ...
    else if(A==4)
    {
      ...
      if(Z!=2) throw G4HadronicException(__FILE__, __LINE__, "ParticleHPContAngularPar: Unknown ion case 1");    
    }
    else
    {
            ...
            if ( theAngular[j].GetLabel() == 0.0 ) {
               ...
               if ( j != nEnergies-1 ) {
                  ...
                  if ( theAngular[j].GetValue(0) != 0.0 ) {
                     throw G4HadronicException(__FILE__, __LINE__, "ParticleHPContAngularPar: Unexpected non zero value of theAngular[nEnergies-1].GetValue(0)");    
                  }
               }
            }
...
//          Discrete Emission 
            for ( G4int j = 0 ; j < nDiscreteEnergies ; j++ )
	    {
 	    ...
 	    ParticleHPLegendreStore theStore(1);
...
//          Continuous Emission
            for ( G4int j = nDiscreteEnergies ; j < nEnergies ; j++ )
	    {
	       ...
	       if ( random < running[ j ] ) 
            ...
            ParticleHPLegendreStore theStore(2);
            ...
            for (G4int j=0;j<nAngularParameters;j++)
            {
               ...
               if ( it == 0 ) 
               {
                  ...
                  //G4cout << "110611 ParticleHPContAngularPar::Sample it = 0; invetigation required " << G4endl;
                  itt = it+1; 
               }
...
*/
    ...
    else if(angularRep==2)
    {
      ...
      running[0]=0;
      G4double weighted = 0;
      if( std::getenv("G4PHPTEST") ) G4cout << "  ParticleHPContAngularPar::Sample nEnergies " << nEnergies << G4endl;
      for(j=1; j<nEnergies; j++)
      {
        ...
        weighted += theInt.GetWeightedBinIntegral(theManager.GetScheme(j-1),
                             theAngular[j-1].GetLabel(), theAngular[j].GetLabel(),
                             theAngular[j-1].GetValue(0), theAngular[j].GetValue(0));
	if( std::getenv("G4PHPTEST") ) G4cout << "  ParticleHPContAngularPar::Sample " << j << " running " << running[j] 
      ...
      ParticleHPKallbachMannSyst theKallbach(compoundFraction,
      ...
      if( std::getenv("G4PHPTEST") ) G4cout << " ParticleHPKallbachMannSyst::Sample resulttest " << cosTh << G4endl; //GDEB
    }
    else if(angularRep>10&&angularRep<16)
    {
      ...
      else
      {
        ...
        fsEnergy = theInt.Interpolate(theManager.GetInverseScheme(it),
                                      random,x1,x2,y1,y2);
        ParticleHPVector theBuff1;
        ParticleHPVector theBuff2;
        G4InterpolationManager aMan;
        aMan.Init(angularRep-10, nAngularParameters-1);
...
//      Bug Report #1366 from L. Russell 
    ...
    {
      throw G4HadronicException(__FILE__, __LINE__, "ParticleHPContAngularPar::Sample: Unknown angular representation");
    }
...
void ParticleHPContAngularPar::PrepareTableInterpolation(const ParticleHPContAngularPar* angParPrev)
{
  ...
  for(ie=nDiscreteEnergies; ie<nEnergies; ie++) {
    ...
    //-    if( getenv("G4PHPTEST2") ) G4cout <<this << " ParticleHPContAngularPar::PrepareTableInterpolation  theEnergiesTransformed1 " << enerT << G4endl; //GDEB
  } 
  ...
  for(ie=nDiscreteEnergiesPrev; ie<nEnergiesPrev; ie++) {
    ...
    theEnergiesTransformed.insert(enerT);
    //-    if( getenv("G4PHPTEST2") ) G4cout << this << " ParticleHPContAngularPar::PrepareTableInterpolation  theEnergiesTransformed2 " << enerT << G4endl; //GDEB
  }
...
void ParticleHPContAngularPar::BuildByInterpolation(G4double anEnergy, G4InterpolationScheme aScheme, 
							ParticleHPContAngularPar & angpar1, 
							ParticleHPContAngularPar & angpar2) 
{
  ...
  for( itede = theDiscreteEnergies.begin(); itede != theDiscreteEnergies.end(); itede++, ie++ ) {
	...
	val2 = 0.;
      ...
      G4double value = theInt.Interpolate(aScheme, anEnergy, 
					  ...
					  val1,
					  val2);
      if( std::getenv("G4PHPTEST2") ) G4cout << ie << " " << ip << " ParticleHPContAngularPar::Merge DiscreteEnergies  val1 " << val1 << " val2 " << val2 << " value " << value << G4endl; //GDEB
  ...
  if( std::getenv("G4PHPTEST2") )  G4cout << " ParticleHPContAngularPar::Merge E " << anEnergy << " minmax " << theMinEner << " " << theMaxEner << G4endl; //GDEB
  ...
  for(ie=nDiscreteEnergies; ie<nEnergies; ie++,iteet++) { 
    ...
    if( std::getenv("G4PHPTEST2") )  G4cout << ie << " " << ie1 << " " << ie2 << " ParticleHPContAngularPar::loop eT " << eT << " -> eN " << eN << " e1 " << e1 << " e2 " << e2 << G4endl; //GDEB
    ...
    for(G4int ip=0; ip<nAngularParameters; ip++) {
      ...
      if ( theMaxEner != theMinEner ) {
         ...
         throw G4HadronicException(__FILE__, __LINE__, "ParticleHPContAngularPar::PrepareTableInterpolation theMaxEner == theMinEner and  value != 0.");
      }
      if( std::getenv("G4PHPTEST2") ) G4cout << ie << " " << ip << " ParticleHPContAngularPar::Merge val1 " << val1 << " val2 " << val2 << " value " << value << G4endl; //GDEB
      //-      val1 = angpar1.theAngular[ie1-1].GetValue(ip) * (maxEner1-minEner1); 
      //-      val2 = angpar2.theAngular[ie2-1].GetValue(ip) * (maxEner2-minEner2); 
      //-      if( getenv("G4PHPTEST2") ) G4cout << ie << " " << ip << " ParticleHPContAngularPar::MergeOLD val1 " << val1 << " val2 " << val2 << " value " << value << G4endl; //GDEB
  ...
  if( std::getenv("G4PHPTEST2") ) {
    G4cout << " ParticleHPContAngularPar::Merge ANGPAR1 " << G4endl; //GDEB
    angpar1.Dump();
    G4cout << " ParticleHPContAngularPar::Merge ANGPAR2 " << G4endl;
    angpar2.Dump();
    G4cout << " ParticleHPContAngularPar::Merge ANGPARNEW " << G4endl;
    Dump();
  }   
}

void ParticleHPContAngularPar::Dump()
{
