...
#include "ParticleHPThermalScatteringData.hh"
...
ParticleHPThermalScatteringData::ParticleHPThermalScatteringData()
...
// Upper limit of neutron energy 
   ...
   names = new ParticleHPThermalScatteringNames();
}
...
G4bool ParticleHPThermalScatteringData::IsIsoApplicable( const G4DynamicParticle* dp , 
                                                G4int /*Z*/ , G4int /*A*/ ,
...
G4double ParticleHPThermalScatteringData::GetIsoCrossSection( const G4DynamicParticle* dp ,
                                   G4int /*Z*/ , G4int /*A*/ ,
                                   const G4Isotope* /*iso*/  ,
                                   const G4Element* element ,
                                   const G4Material* material )
...
void ParticleHPThermalScatteringData::clearCurrentXSData()
{
   std::map< G4int , std::map< G4double , ParticleHPVector* >* >::iterator it;
   std::map< G4double , ParticleHPVector* >::iterator itt;
...
G4bool ParticleHPThermalScatteringData::IsApplicable( const G4DynamicParticle* aP , const G4Element* anEle )
{
   G4bool result = false;
...
void ParticleHPThermalScatteringData::BuildPhysicsTable(const G4ParticleDefinition& aP)
{
   ...
   if ( G4Threading::IsMasterThread() ) {
      ...
      if ( coherent == NULL ) coherent = new std::map< G4int , std::map< G4double , ParticleHPVector* >* >;
      if ( incoherent == NULL ) incoherent = new std::map< G4int , std::map< G4double , ParticleHPVector* >* >;
      if ( inelastic == NULL ) inelastic = new std::map< G4int , std::map< G4double , ParticleHPVector* >* >;
      ...
      for ( std::map < G4String , G4int >::iterator it = co_dic.begin() ; it != co_dic.end() ; it++ )  
      {
         ...
         std::map< G4double , ParticleHPVector* >*  coh_amapTemp_EnergyCross = readData( full_name );
         coherent->insert ( std::pair < G4int , std::map< G4double , ParticleHPVector* >* > ( ts_ID , coh_amapTemp_EnergyCross ) );

         // Incoherent
         full_name = dirName + "/Incoherent/CrossSection/" + ndl_filename; 
         std::map< G4double , ParticleHPVector* >*  incoh_amapTemp_EnergyCross = readData( full_name );
         incoherent->insert ( std::pair < G4int , std::map< G4double , ParticleHPVector* >* > ( ts_ID , incoh_amapTemp_EnergyCross ) );

         // Inelastic
         full_name = dirName + "/Inelastic/CrossSection/" + ndl_filename; 
         std::map< G4double , ParticleHPVector* >*  inela_amapTemp_EnergyCross = readData( full_name );
         inelastic->insert ( std::pair < G4int , std::map< G4double , ParticleHPVector* >* > ( ts_ID , inela_amapTemp_EnergyCross ) );
...
std::map< G4double , ParticleHPVector* >* ParticleHPThermalScatteringData::readData ( G4String full_name ) 
{

   std::map< G4double , ParticleHPVector* >*  aData = new std::map< G4double , ParticleHPVector* >; 
   
   //std::ifstream theChannel( full_name.c_str() );
   std::istringstream theChannel;
   ParticleHPManager::GetInstance()->GetDataStream(full_name,theChannel);

   //G4cout << "ParticleHPThermalScatteringData " << name << G4endl;
   ...
   while ( theChannel >> dummy )   // MF // Loop checking, 11.05.2015, T. Koi
   {
      ...
      G4double temp; 
      theChannel >> temp;   
      ParticleHPVector* anEnergyCross = new ParticleHPVector;
      G4int nData;
      theChannel >> nData;
      anEnergyCross->Init ( theChannel , nData , eV , barn );
      aData->insert ( std::pair < G4double , ParticleHPVector* > ( temp , anEnergyCross ) );
   }
   //theChannel.close();
...
void ParticleHPThermalScatteringData::DumpPhysicsTable( const G4ParticleDefinition& aP )
{
   if( &aP != G4Neutron::Neutron() ) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");  
//  G4cout << "ParticleHPThermalScatteringData::DumpPhysicsTable still to be implemented"<<G4endl;
}
...
/*
G4double ParticleHPThermalScatteringData::GetCrossSection( const G4DynamicParticle* aP , const G4Element*anE , G4double aT )
{
   ...
   //G4cout << "ParticleHPThermalScatteringData::GetCrossSection  Tot= " << result/barn << " Coherent= " << Xcoh/barn << " Incoherent= " << Xincoh/barn << " Inelastic= " << Xinela/barn << G4endl;
...
G4double ParticleHPThermalScatteringData::GetCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
...
G4double ParticleHPThermalScatteringData::GetInelasticCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
   G4int ts_id = getTS_ID( aM , anE );
   G4double aT = aM->GetTemperature();
...
G4double ParticleHPThermalScatteringData::GetCoherentCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
   G4int ts_id = getTS_ID( aM , anE );
   G4double aT = aM->GetTemperature();
...
G4double ParticleHPThermalScatteringData::GetIncoherentCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
   G4int ts_id = getTS_ID( aM , anE );
   G4double aT = aM->GetTemperature();
...
G4int ParticleHPThermalScatteringData::getTS_ID ( const G4Material* material , const G4Element* element )
{
   G4int result = -1;
   if ( dic.find( std::pair < const G4Material* , const G4Element* > ( (G4Material*)NULL , element ) ) != dic.end() ) 
...
G4double ParticleHPThermalScatteringData::GetX ( const G4DynamicParticle* aP, G4double aT , std::map < G4double , ParticleHPVector* >* amapTemp_EnergyCross )
{
   ...
   if ( amapTemp_EnergyCross->size() == 1 ) { 
      if ( std::fabs ( aT - amapTemp_EnergyCross->begin()->first ) / amapTemp_EnergyCross->begin()->first > 0.1 ) {
         G4cout << "ParticleHPThermalScatteringData:: The temperature of material (" 
   ...
   std::map< G4double , ParticleHPVector* >::iterator it; 
   ...
   //G4cout << "ParticleHPThermalScatteringData::GetX TH " << TH << " E " << eKinetic <<  " XH " << XH << G4endl;

   if ( it != amapTemp_EnergyCross->begin() ) it--;
   G4double TL = it->first;
   G4double XL = it->second->GetXsec ( eKinetic ); 

   //G4cout << "ParticleHPThermalScatteringData::GetX TL " << TL << " E " << eKinetic <<  " XL " << XL << G4endl;
...
void ParticleHPThermalScatteringData::AddUserThermalScatteringFile( G4String nameG4Element , G4String filename )
{
   names->AddThermalElement( nameG4Element , filename );
}
void ParticleHPThermalScatteringData::CrossSectionDescription(std::ostream& outFile) const
{
