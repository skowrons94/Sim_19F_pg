...
//
#include "ParticleHPField.hh"
#include "G4HadronicException.hh"
#include "G4ios.hh"


  ParticleHPField::ParticleHPField()
  {
    theData = new ParticleHPFieldPoint[100]; 
    nPoints=100;
    nEntries=0;
    theData->SetData(0,0,0);
  }
  
  ParticleHPField::~ParticleHPField(){ delete [] theData;}
  
  G4double ParticleHPField::GetY(G4double e, G4int j)
  {
    G4int low   = 0;
    G4int high  = 0;
    G4int i;
  ...
  void ParticleHPField::Check(G4int i)
  {
    if(i>nEntries) throw G4HadronicException(__FILE__, __LINE__, "Skipped some index numbers in ParticleHPField");
    if(i==nPoints)
    {
      nPoints += 50;
      ParticleHPFieldPoint * buff = new ParticleHPFieldPoint[nPoints];
