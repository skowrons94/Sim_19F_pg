...
ParticleHPElasticData::ParticleHPElasticData()
...
G4bool ParticleHPElasticData::IsIsoApplicable( const G4DynamicParticle* dp , 
                                                G4int /*Z*/ , G4int /*A*/ ,
                                                const G4Element* /*elm*/ ,
...
G4double ParticleHPElasticData::GetIsoCrossSection( const G4DynamicParticle* dp ,
                                   G4int /*Z*/ , G4int /*A*/ ,
                                   const G4Isotope* /*iso*/  ,
...
/*
G4bool ParticleHPElasticData::IsApplicable(const G4DynamicParticle*aP, const G4Element*)
{
  G4bool result = true;
...
void ParticleHPElasticData::BuildPhysicsTable(const G4ParticleDefinition& aP)
{
  ...
  if(&aP!=G4Neutron::Neutron()) 
   ...
   if ( G4Threading::IsWorkerThread() ) {
      theCrossSections = ParticleHPManager::GetInstance()->GetElasticCrossSections();
      return;
   }
...
// TKDB
  ...
  for( size_t i=0; i<numberOfElements; ++i )
  {
   ...
   ParticleHPManager::GetInstance()->RegisterElasticCrossSections(theCrossSections);
}

void ParticleHPElasticData::DumpPhysicsTable(const G4ParticleDefinition& aP)
{
  if(&aP!=G4Neutron::Neutron()) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");  
...
//
   ...
   //G4cout << "ParticleHPElasticData::DumpPhysicsTable still to be implemented"<<G4endl;
   #endif
}
...
G4double ParticleHPElasticData::
