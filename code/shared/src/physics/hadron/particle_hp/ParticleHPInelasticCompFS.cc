...
//
#include "ParticleHPInelasticCompFS.hh"
...
void ParticleHPInelasticCompFS::InitGammas(G4double AR, G4double ZR)
{
  //   char the[100] = {""};
...
void ParticleHPInelasticCompFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String & aFSType, G4ParticleDefinition*)
{
  ...
  G4bool dbool;
  ParticleHPDataUsed aFile = theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M, tString, aFSType, dbool);
  G4String filename = aFile.GetName();
#ifdef G4PHPDEBUG
  if( std::getenv("ParticleHPDebug") ) G4cout << " ParticleHPInelasticCompFS::Init FILE " << filename << G4endl;
#endif
...
//std::ifstream theData(filename, std::ios::in);
  ...
  while (theData >> infoType) // Loop checking, 11.05.2015, T. Koi
  {
    ...
    if(dataType==3) 
    {
      ...
      QI[ it ] = dqi*CLHEP::eV;
      LR[ it ] = ilr;
      theXsection[it] = new ParticleHPVector;
      G4int total;
      theData >> total;
    ...
    {
      theAngularDistribution[it] = new ParticleHPAngular;
      theAngularDistribution[it]->Init(theData);
    }
    else if(dataType==5)
    {
      theEnergyDistribution[it] = new ParticleHPEnergyDistribution;
      theEnergyDistribution[it]->Init(theData); 
    }
    else if(dataType==6)
    {
      theEnergyAngData[it] = new ParticleHPEnAngCorrelation(theProjectile);
      //      G4cout << this << " CompFS theEnergyAngData " << it << theEnergyAngData[it] << G4endl; //GDEB
      theEnergyAngData[it]->Init(theData);
    }
    else if(dataType==12)
    {
      theFinalStatePhotons[it] = new ParticleHPPhotonDist;
    ...
    {
      throw G4HadronicException(__FILE__, __LINE__, "Data-type unknown to ParticleHPInelasticCompFS");
    }
  }
  //  theData.close();
}

G4int ParticleHPInelasticCompFS::SelectExitChannel(G4double eKinetic)
{
...
// n,p,d,t,he3,a
void ParticleHPInelasticCompFS::CompositeApply(const G4HadProjectile& theTrack,
...
#ifdef G4PHPDEBUG
    if( std::getenv("ParticleHPDebug"))  G4cout <<this <<" ParticleHPInelasticCompFS::CompositeApply A " <<theBaseA <<" Z " <<theBaseZ <<" incident " <<hadProjectile->GetDefinition()->GetParticleName() <<G4endl;
#endif
...
// select exit channel for composite FS class.
   ...
   if ( ParticleHPManager::GetInstance()->GetUseNRESP71Model() && eKinetic<20*MeV) {
...
//    TK Excitation level is not determined
	...
	*/
	...
	if(std::getenv("ParticleHPDebug") && eKinetic-eExcitation < 0) 
	{
...
// This needs more design @@@
...
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticCompFS::BaseApply  add secondary1 " << theSec->GetParticleDefinition()->GetParticleName() << " E= " << theSec->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
      ...
      if(thePhotons!=0)
	{
      ...
      theResult.Get()->AddSecondary(theSec, secID);    
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticCompFS::BaseApply add secondary2 " << theSec->GetParticleDefinition()->GetParticleName() << " E= " << theSec->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
    }
    else
    {
      for(i0=0; i0<theParticles->size(); i0++)
      {
        ...
        theResult.Get()->AddSecondary(theSec, secID); 
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticCompFS::BaseApply add secondary3 " << theSec->GetParticleDefinition()->GetParticleName() << " E= " << theSec->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
	...
	//        theResidual.SetDefinition(G4IonTable::GetIonTable()
        //                          ->GetIon(static_cast<G4int>(residualZ), static_cast<G4int>(residualA), 0));
        theResidual.SetDefinition(G4IonTable::GetIonTable()
	                          ->GetIon(static_cast<G4int>(residualZ), static_cast<G4int>(residualA), G4NuclearLevelData::GetInstance()->GetLevelEnergy(static_cast<G4int>(residualZ),static_cast<G4int>(residualA),QI[0]-QI[it])));  
...
//080717 TK Comment still do NOT include photon's mometum which produce by thePhotons
        ...
        theResult.Get()->AddSecondary(theSec, secID);  
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticCompFS::BaseApply add secondary4 " << theSec->GetParticleDefinition()->GetParticleName() << " E= " << theSec->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
    ...
    if(thePhotons!=0)
    {
      for(i=0; i<nPhotons; i++)
      {
        ...
        theResult.Get()->AddSecondary(theSec, secID); 
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticCompFS::BaseApply add secondary5 " << theSec->GetParticleDefinition()->GetParticleName() << " E= " << theSec->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
...
void ParticleHPInelasticCompFS::two_body_reaction(G4ReactionProduct* proj,
...
G4bool ParticleHPInelasticCompFS::use_nresp71_model( const G4ParticleDefinition* aDefinition , const G4int it , const G4ReactionProduct& theTarget , G4ReactionProduct& boosted )
{
	if ( aDefinition == G4Neutron::Definition() ) // If the outgoing particle is a neutron...
		...
		if ( LR[it] == 0 ) // If Z=6, an alpha particle is emitted and there is no breakup of the residual nucleus LR(flag LR in ENDF)==0.
			...
			{
			G4Exception("ParticleHPInelasticCompFS::CompositeApply()", "G4ParticleInelasticCompFS.cc", FatalException, "Alpha production with LR!=0.");
			}
	}
