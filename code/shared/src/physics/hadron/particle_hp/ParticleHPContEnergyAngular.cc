...
//
#include "ParticleHPContEnergyAngular.hh"

G4ReactionProduct * ParticleHPContEnergyAngular::Sample(G4double anEnergy, G4double massCode, G4double /*mass*/)
{
   ...
   if( std::getenv("G4PHPTEST") )    G4cout << i << " ParticleHPContEnergyAngular dataE " << theAngular[i].GetEnergy() << " > " << anEnergy << " it_theAngular " << it << " interpolation " << theInterpolation << G4endl; //GDEB
   ...
   else
   {
     ...
     //--- create new 
     //     if( theManager.GetScheme(0) != LINLIN ) {  // asserted in ParticleHPContEnergyAngular::init there is only one range
#ifdef PHP_AS_HP
...
#else
    if( std::getenv("G4PHPTEST") )     G4cout << i << " ParticleHPContEnergyAngular To BUILDBYINTERPOLATION " << it << " : " << theAngular[it].GetEnergy()<< " , " << theAngular[it].GetNEnergies() << " " << it-1 << " : " << theAngular[it-1].GetEnergy()<< " : " << theAngular[it-1].GetNEnergies() << G4endl; //GDEB

//     ParticleHPContAngularPar * fAngular = new  ParticleHPContAngularPar(theProjectile ); //fix start
     if (fCacheAngular.Get() == NULL) {
		ParticleHPContAngularPar* angpar = new ParticleHPContAngularPar(theProjectile);
...
//     delete fAngular; //fix end
   ...
   //      G4cout << " 0 0 @@@ ParticleHPContEnergyAngular::Sample " << result->GetDefinition()->GetParticleName() << " E= " << result->GetKineticEnergy() << G4endl;//GDEB
   return result;
}

G4double ParticleHPContEnergyAngular::
MeanEnergyOfThisInteraction()
{
   G4double result(0);
   if(currentMeanEnergy.Get()<-1)
   {
     throw G4HadronicException(__FILE__, __LINE__, "ParticleHPContEnergyAngular: Logical error in Product class");
   }
   else
   {
...
void ParticleHPContEnergyAngular::ClearHistories()
{ 
   if ( theAngular!= NULL )
   { 
