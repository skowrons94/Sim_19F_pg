...
//
#include "ParticleHPThermalScattering.hh"
#include "ParticleHPThermalScatteringData.hh"
#include "ParticleHPThermalScatteringNames.hh"
#include "ParticleHPElastic.hh"
...
ParticleHPThermalScattering::ParticleHPThermalScattering()
                             :G4HadronicInteraction("NeutronHPThermalScattering")
,coherentFSs(nullptr)
,incoherentFSs(nullptr)
,inelasticFSs(nullptr)
{
   theHPElastic = new ParticleHPElastic();

   SetMinEnergy( 0.*eV );
   SetMaxEnergy( 4*eV );
   theXSection = new ParticleHPThermalScatteringData();
...
void ParticleHPThermalScattering::clearCurrentFSData() {
...
void ParticleHPThermalScattering::BuildPhysicsTable(const G4ParticleDefinition& particle) {
   buildPhysicsTable();
   theHPElastic->BuildPhysicsTable( particle );
}


std::map < G4double , std::vector < std::pair< G4double , G4double >* >* >* ParticleHPThermalScattering::readACoherentFSDATA( G4String name )
{
   ...
   ParticleHPManager::GetInstance()->GetDataStream(name,theChannel);
...
std::map < G4double , std::vector < E_P_E_isoAng* >* >* ParticleHPThermalScattering::readAnInelasticFSDATA ( G4String name )
{
...
E_P_E_isoAng* ParticleHPThermalScattering::readAnE_P_E_isoAng( std::istream* file ) // for inelastic
{
   ...
   *file >> nl;
   ...
   for ( G4int i = 0 ; i < aData->n ; i++ )
   {
      ...
      *file >> prob;
      ...
      //G4cout << "ParticleHPThermalScattering inelastic " << energy/eV << " " <<  i << " " << prob << " " << aData->prob[ i ] << G4endl; 
      for ( G4int j = 0 ; j < anE_isoAng->n ; j++ )
      {
         G4double x;
         *file >> x;
         anE_isoAng->isoAngle[j] = x ;
         //G4cout << "ParticleHPThermalScattering inelastic " << x << anE_isoAng->isoAngle[j] << G4endl; 
      }
   } 
...
std::map < G4double , std::vector < E_isoAng* >* >* ParticleHPThermalScattering::readAnIncoherentFSDATA ( G4String name )
{
...
E_isoAng* ParticleHPThermalScattering::readAnE_isoAng( std::istream* file )
{
...
G4HadFinalState* ParticleHPThermalScattering::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
{
...
//--------------------------------------------------
std::pair< G4double , G4int> ParticleHPThermalScattering::
...
// New method added by L. Thulliez 2021 (CEA-Saclay)
//--------------------------------------------------
std::pair< G4double , G4double > ParticleHPThermalScattering::
...
// New method added by L. Thulliez 2021 (CEA-Saclay)
//--------------------------------------------------
G4double ParticleHPThermalScattering::getMu( G4double rndm1, G4double rndm2, E_isoAng* anEPM )
{
   G4double result = 0.0;
...
G4double ParticleHPThermalScattering::getMu( E_isoAng* anEPM  )
{
...
std::pair < G4double , G4double >  ParticleHPThermalScattering::find_LH ( G4double x , std::vector< G4double >* aVector )
{
   G4double LL = 0.0; 
   G4double H = 0.0; 
...
G4double ParticleHPThermalScattering::get_linear_interpolated ( G4double x , std::pair< G4double , G4double > Low , std::pair< G4double , G4double > High )
{ 
   G4double y=0.0;
   if ( High.first - Low.first != 0 ) {
      ...
      if ( High.second == Low.second ) {
         ...
         G4cout << "ParticleHPThermalScattering liner interpolation err!!" << G4endl; 
      }
   }
...
E_isoAng
ParticleHPThermalScattering::create_E_isoAng_from_energy(G4double energy,
                                                           std::vector<E_isoAng*>* vEPM)
{
   ...
   if (panEPM_T_EL != 0 && panEPM_T_EH != 0) {
     ...
     if (panEPM_T_EL->n == panEPM_T_EH->n) {
       ...
       G4Exception("ParticleHPThermalScattering::create_E_isoAng_from_energy",
                   "NotSupported", JustWarning,
                   "ParticleHPThermalScattering does not support yet EL->n != EH->n."); 
     }
...
G4double ParticleHPThermalScattering::get_secondary_energy_from_E_P_E_isoAng ( G4double random , E_P_E_isoAng* anE_P_E_isoAng )
{
...
std::pair< G4double , E_isoAng > ParticleHPThermalScattering::create_sE_and_EPM_from_pE_and_vE_P_E_isoAng ( G4double rand_for_sE ,  G4double pE , std::vector < E_P_E_isoAng* >*  vNEP_EPM )
{
   std::map< G4double , G4int > map_energy;
   map_energy.clear();
   std::vector< G4double > v_energy;
...
void ParticleHPThermalScattering::buildPhysicsTable()
{
...
G4int ParticleHPThermalScattering::getTS_ID ( const G4Material* material , const G4Element* element )
{
   G4int result = -1;
...
const std::pair<G4double, G4double> ParticleHPThermalScattering::GetFatalEnergyCheckLevels() const
{
...
void ParticleHPThermalScattering::AddUserThermalScatteringFile( G4String nameG4Element , G4String filename)
{
...
G4bool ParticleHPThermalScattering::check_E_isoAng( E_isoAng* anE_IsoAng )
{
   G4bool result=false;
...
void ParticleHPThermalScattering::ModelDescription(std::ostream& outFile) const
{
