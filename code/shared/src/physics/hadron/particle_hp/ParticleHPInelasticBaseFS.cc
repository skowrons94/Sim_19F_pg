...
//
// June-2019 - E. Mendoza --> Added protection against residual with Z<0 or A<Z + adjust_final_state is not applied when data is in MF=6 format (no correlated particle emission) + bug correction (add Q value info to ParticleHPNBodyPhaseSpace).
...
#include "ParticleHPDataUsed.hh"

#include "G4IonTable.hh"

void ParticleHPInelasticBaseFS::InitGammas(G4double AR, G4double ZR)
{
  //   char the[100] = {""};
...
void ParticleHPInelasticBaseFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String & bit, G4ParticleDefinition* )
{
  ...
  G4bool dbool;
  ParticleHPDataUsed aFile = theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M,tString, bit, dbool);
  G4String filename = aFile.GetName();
#ifdef G4PHPDEBUG
  if( std::getenv("ParticleHPDebug") ) G4cout << " ParticleHPInelasticBaseFS::Init FILE " << filename << G4endl;
#endif
   SetAZMs( A, Z, M, aFile); 
  //theBaseA = aFile.GetA();
  //theBaseZ = aFile.GetZ();
  ...
  {
#ifdef G4PHPDEBUG
    if(std::getenv("ParticleHPDebug_NamesLogging")) G4cout << "Skipped = "<< filename <<" "<<A<<" "<<Z<<G4endl;
#endif
    hasAnyData = false;
    hasFSData = false; 
    hasXsec = false;
  ...
  //std::ifstream theData(filename, std::ios::in);
   ...
   ParticleHPManager::GetInstance()->GetDataStream(filename,theData);
   //130205 END
  ...
  while (theData >> infoType) // Loop checking, 11.05.2015, T. Koi
  {
    ...
    {
      theAngularDistribution = new ParticleHPAngular;
      theAngularDistribution->Init(theData);
      hasFSData = true; 
    }
    else if(dataType==5)
    {
      theEnergyDistribution = new ParticleHPEnergyDistribution;
      theEnergyDistribution->Init(theData);
      hasFSData = true; 
    }
    else if(dataType==6)
    {
      theEnergyAngData = new ParticleHPEnAngCorrelation(theProjectile);
    ...
    {
      theFinalStatePhotons = new ParticleHPPhotonDist;
    ...
    {
      throw G4HadronicException(__FILE__, __LINE__, "Data-type unknown to ParticleHPInelasticBaseFS");
    }
  }
  //  theData.close();
}
  
void ParticleHPInelasticBaseFS::BaseApply(const G4HadProjectile & theTrack, 
...
// Take N-body phase-space distribution, if no other data present.
  ...
  {
    ParticleHPNBodyPhaseSpace thePhaseSpaceDistribution;
...
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticBaseFS::BaseApply NoFSData add secondary " << aPart->GetParticleDefinition()->GetParticleName() << " E= " << aPart->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
    }   
...
// set target and neutron in the relevant exit channel
  ...
  if(theEnergyAngData != 0)
  {
    ...
    if ( ! ParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) {
      //141017 Fix BEGIN 
...
//      }
#ifdef G4PHPDEBUG
//      if( getenv("ParticleHPDebug"))  G4cout << " ParticleHPInelasticBaseFS::BaseApply secondary previously added " << tmpHadrons->operator[](i)->GetDefinition()->GetParticleName() << " E= " << tmpHadrons->operator[](i)->GetKineticEnergy() << G4endl;
#endif
//    }
	...
	if(theEnergyDistribution==0 && nDef == 2)
	{
...
#ifdef G4PHPDEBUG
	    if(std::getenv("ParticleHPDebug")) // @@@@@ verify the nucleon counting...
	    { 
	      G4cout << "ParticleHPInelasticBaseFS "<<z1<<" "<<theBaseZ<<" "<<a1<<" "<<theBaseA<<" "<<availableEnergy<<" "
	             << emin<<G4endl;
            }
#endif
	  }
...
#ifdef G4PHPDEBUG
	    if(std::getenv("ParticleHPDebug")) 
	  {
	    G4cout << " ParticleHPInelasticBaseFS "<<bufferedDirection.mag2()<<G4endl;
	  }
#endif
	  ...
	  aHadron->SetMomentum(bufferedDirection);
	    ...
	    if(std::getenv("ParticleHPDebug"))
	  {
	    G4cout << " ParticleHPInelasticBaseFS "<<aHadron->GetTotalEnergy()<<" "<<aHadron->GetMomentum()<<G4endl;
	  }
#endif
	}
	tmpHadrons->push_back(aHadron);
#ifdef G4PHPDEBUG
	if( std::getenv("ParticleHPDebug"))  G4cout << " ParticleHPInelasticBaseFS::BaseApply FSData add secondary " << aHadron->GetDefinition()->GetParticleName() << " E= " << aHadron->GetKineticEnergy() << G4endl;
#endif
      }
    }
    delete [] Done;
  ...
  else if(theEnergyAngData!=0)
  {
   ...
   // PA130927: do not create photons to adjust binding energy
    ...
    bAdjustPhotons = true; 
#else
    if ( ParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) bAdjustPhotons = false;    
#endif
...
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << " ParticleHPInelasticBaseFS::BaseApply gamma Energy " << theGammaEnergy << " eBindProducts " << eBindProducts << G4endl;
#endif
...
#ifdef G4PHPDEBUG
	      if( std::getenv("ParticleHPDebug"))  G4cout << " ParticleHPInelasticBaseFS::BaseApply adding gamma from level " << iLevel << theOtherPhotons->operator[](ii)->GetKineticEnergy() << G4endl;
#endif
	    }
	  else
	    {
...
#ifdef G4PHPDEBUG
	  if( std::getenv("ParticleHPDebug"))
	        G4cout << iii << " ParticleHPInelasticBaseFS::BaseApply adding gamma " << theOtherPhotons->operator[](iii)->GetKineticEnergy() << G4endl;
#endif
		}
...
#ifdef G4PHPDEBUG
  if( std::getenv("ParticleHPDebug"))  G4cout << " ParticleHPInelasticBaseFS::BaseApply N hadrons " << nSecondaries-nPhotons << G4endl;
#endif
...
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticBaseFS::BaseApply add secondary2 " << theSec->GetParticleDefinition()->GetParticleName() << " E= " << theSec->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
...
#ifdef G4PHPDEBUG
  if( std::getenv("ParticleHPDebug"))  G4cout << " ParticleHPInelasticBaseFS::BaseApply N photons " << nPhotons << G4endl;
#endif
  if(thePhotons != 0)
  {
    for(i=0; i<nPhotons; i++)
    {
      ...
      theResult.Get()->AddSecondary(theSec, secID); 
#ifdef G4PHPDEBUG
      if( std::getenv("ParticleHPDebug"))  G4cout << this << " ParticleHPInelasticBaseFS::BaseApply add secondary3 " << theSec->GetParticleDefinition()->GetParticleName() << " E= " << theSec->GetKineticEnergy() << " NSECO " << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
