...
ParticleHPData::ParticleHPData(G4ParticleDefinition* projectile )
    : theProjectile(projectile)
{
  ...
  if( projectile == G4Neutron::Neutron() ) {
    ...
    theDataDirVariable = "G4C12HPDATA";
  }

     numEle = G4Element::GetNumberOfElements();
     for ( G4int i = 0 ; i < numEle ; i++ ) theData.push_back ( new ParticleHPElementData );
//     G4cout << "ParticleHPData::ParticleHPData(): numEle="<<numEle<<G4endl;
  ...
  ParticleHPData::~ParticleHPData()
  {
     for ( std::vector<ParticleHPElementData*>::iterator it = theData.begin() ; it != theData.end() ; it++ ) delete *it;
     theData.clear();
  }
  
  ParticleHPData * ParticleHPData::Instance(G4ParticleDefinition* projectile)
  {
    static G4ThreadLocal ParticleHPData *theCrossSectionData_G4MT_TLS_ = 0 ;
    if ( !theCrossSectionData_G4MT_TLS_ ) theCrossSectionData_G4MT_TLS_ = new ParticleHPData(projectile);  
    ParticleHPData &theCrossSectionData = *theCrossSectionData_G4MT_TLS_;
    return &theCrossSectionData;
  } 

  G4PhysicsVector * ParticleHPData::DoPhysicsVector(ParticleHPVector * theVector)
  {
//    G4cout << "Entered ParticleHPData::DoPhysicsVector."<<G4endl;
...
void ParticleHPData::addPhysicsVector()
{
   for ( G4int i = numEle; i < (G4int)G4Element::GetNumberOfElements() ; i++ )
   {
      theData.push_back ( new ParticleHPElementData );
