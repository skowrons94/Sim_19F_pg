...
ParticleHPFissionData::ParticleHPFissionData()
...
G4bool ParticleHPFissionData::IsIsoApplicable( const G4DynamicParticle* dp , 
                                                G4int /*Z*/ , G4int /*A*/ ,
                                                const G4Element* /*elm*/ ,
...
G4double ParticleHPFissionData::GetIsoCrossSection( const G4DynamicParticle* dp ,
                                   G4int /*Z*/ , G4int /*A*/ ,
                                   const G4Isotope* /*iso*/  ,
...
/*
G4bool ParticleHPFissionData::IsApplicable(const G4DynamicParticle*aP, const G4Element*)
{
  G4bool result = true;
...
void ParticleHPFissionData::BuildPhysicsTable(const G4ParticleDefinition& aP)
{
  if(&aP!=G4Neutron::Neutron()) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");  

   if ( G4Threading::IsWorkerThread() ) {
      theCrossSections = ParticleHPManager::GetInstance()->GetFissionCrossSections();
      return;
   }
  ...
  for( size_t i=0; i<numberOfElements; ++i )
  {
   ...
   ParticleHPManager::GetInstance()->RegisterFissionCrossSections( theCrossSections );
}

void ParticleHPFissionData::DumpPhysicsTable(const G4ParticleDefinition& aP)
{
  if(&aP!=G4Neutron::Neutron()) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");
...
//
  ...
  //G4cout << "ParticleHPFissionData::DumpPhysicsTable still to be implemented"<<G4endl;
  #endif
}

#include "G4NucleiProperties.hh"

G4double ParticleHPFissionData::
...
G4int ParticleHPFissionData::GetVerboseLevel() const 
{
   return ParticleHPManager::GetInstance()->GetVerboseLevel();
}
void ParticleHPFissionData::SetVerboseLevel( G4int newValue ) 
{
   ParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}
void ParticleHPFissionData::CrossSectionDescription(std::ostream& outFile) const
{
