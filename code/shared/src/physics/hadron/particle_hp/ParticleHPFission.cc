...
//
#include "ParticleHPFission.hh"
#include "ParticleHPFissionFS.hh"
#include "G4SystemOfUnits.hh"
#include "ParticleHPManager.hh"
#include "G4Threading.hh"

  ParticleHPFission::ParticleHPFission()
...
/*
    ...
    //theFission = new ParticleHPChannel[numEle];
    ...
    for ( G4int i = 0 ; i < numEle ; i++ ) 
    {
      theFission.push_back( new ParticleHPChannel );
      if((*(G4Element::GetElementTable()))[i]->GetZ()>87) //TK modified for ENDF-VII
      {
       (*theFission[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
       (*theFission[i]).Register(&theFS);
...
*/
  ...
  G4HadFinalState * ParticleHPFission::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
...
const std::pair<G4double, G4double> ParticleHPFission::GetFatalEnergyCheckLevels() const
{
...
/*
void ParticleHPFission::addChannelForNewElement()
{
   for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) 
   {
      ...
      if ( (*(G4Element::GetElementTable()))[i]->GetZ() > 87 ) //TK modified for ENDF-VII
      {
         G4cout << "ParticleHPFission Prepairing Data for the new element of " << (*(G4Element::GetElementTable()))[i]->GetName() << G4endl;
         (*theFission[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
         (*theFission[i]).Register(&theFS);
      }
   }
...
G4int ParticleHPFission::GetVerboseLevel() const
{
   return ParticleHPManager::GetInstance()->GetVerboseLevel();
}
void ParticleHPFission::SetVerboseLevel( G4int newValue ) 
{
   ParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}
void ParticleHPFission::BuildPhysicsTable(const G4ParticleDefinition&)
{
   ...
   if ( G4Threading::IsMasterThread() ) {
      ...
      if ( theFission == NULL ) theFission = new std::vector<ParticleHPChannel*>;
      ...
      for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) {
         ...
         if ((*(G4Element::GetElementTable()))[i]->GetZ()>87) { //TK modified for ENDF-VII
            ((*theFission)[i])->Init((*(G4Element::GetElementTable()))[i], dirName);
            ((*theFission)[i])->Register( new ParticleHPFissionFS );
         }
      }
...
void ParticleHPFission::ModelDescription(std::ostream& outFile) const
{
