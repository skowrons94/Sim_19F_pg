...
#include <fstream>
  ...
  const G4String ParticleHPNames::theString[100] = {"Hydrogen", "Helium",
...
G4String ParticleHPNames::GetName(G4int i) { return theString[i]; }

ParticleHPDataUsed ParticleHPNames::GetName(G4int A, G4int Z, G4int M, G4String base, G4String rest, G4bool & aFlag)
{
  #ifdef G4VERBOSE
  G4int verboseLevel = ParticleHPManager::GetInstance()->GetVerboseLevel();
  #endif
  ...
  do   
    {
       ...
       *biff = base+"/CrossSection/"+itoa(myZ)+"_"+itoa(myA)+sM+"_"+theString[myZ-1];
       ...
       ParticleHPManager::GetInstance()->GetDataStream2(*theName,*check);
       if ( !(*check) ) 
       {
	  //check->close();
	  delete check;
       ...
       else
       {
          ...
          */
          ...
          if ( rest == "/CrossSection" )
          {
            ...
            ParticleHPManager::GetInstance()->GetDataStream2(fileName,*file);
       ...
       do 
       {
         if (delta_Z > theMaxOffSet) {
           if (!ParticleHPManager::GetInstance()->GetSkipMissingIsotopes()) {
             #ifdef G4VERBOSE
             if ( G4HadronicParameters::Instance()->GetVerboseLevel() > 0 ) {
	       G4cout << "ParticleHPNames: There are no data available for some isotopes in this material " << G4endl;
	       G4cout << "ParticleHPNames: nor are there data for nearby isotopes." << G4endl;
	       G4cout << "ParticleHPNames: Please make sure G4NEUTRONHPDATA points to the directory " << G4endl;
	       G4cout << "ParticleHPNames: in which the neutron scattering data are located." << G4endl;
	       G4cout << "ParticleHPNames: The material was A = " << A << ", Z = " << Z << G4endl;
	     }
             #endif
    ...
    if ( Z != result.GetZ() || A != result.GetA() )
    {
       if ( rest == "/CrossSection" )
       {
          ...
          if ( ParticleHPManager::GetInstance()->GetSkipMissingIsotopes() && !( Z == result.GetZ() && result.IsThisNaturalAbundance() ) )
          {
             #ifdef G4VERBOSE
