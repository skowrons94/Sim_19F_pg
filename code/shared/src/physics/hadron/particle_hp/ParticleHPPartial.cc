...
ParticleHPVector * ParticleHPPartial::GetY(G4double e1)
  {
    ParticleHPVector * aBuffer = new ParticleHPVector();
    G4int i;
    if(nData==1)
    {
  ...
  G4double ParticleHPPartial::Sample(G4double x)
  {
    G4int i;
    for (i=0; i<nData; i++)
    {
