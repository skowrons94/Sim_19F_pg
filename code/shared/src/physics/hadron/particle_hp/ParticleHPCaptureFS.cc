...
ParticleHPCaptureFS::ParticleHPCaptureFS()
  {
  ...
  G4HadFinalState * ParticleHPCaptureFS::ApplyYourself(const G4HadProjectile & theTrack)
  {
...
// prepare neutron
    ...
    if ( HasFSData() && !ParticleHPManager::GetInstance()->GetUseOnlyPhotoEvaporation() ) 
    { 
...
//Add photons
       ...
       {
              //G4cout << "TKDB ParticleHPCaptureFS::ApplyYourself we will create additional " << nPhotons - nNonZero << " photons" << G4endl;
...
#include <sstream> 
  void ParticleHPCaptureFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String &, G4ParticleDefinition* )
  {
   ...
   ParticleHPManager::GetInstance()->GetDataStream(filenameMF6,theData);
