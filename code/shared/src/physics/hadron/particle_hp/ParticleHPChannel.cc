...
G4double ParticleHPChannel::GetXsec(G4double energy)
{
  return std::max(0., theChannelData->GetXsec(energy));
}
  
G4double ParticleHPChannel::GetWeightedXsec(G4double energy, G4int isoNumber)
{
  return theIsotopeWiseData[isoNumber].GetXsec(energy);
}
  
G4double ParticleHPChannel::GetFSCrossSection(G4double energy, G4int isoNumber)
{
  return theFinalStates[isoNumber]->GetXsec(energy);
}
  
  void ParticleHPChannel::
  ...
  void ParticleHPChannel::Init(G4Element * anElement, const G4String dirName)  
  {
    theDir = dirName;
    theElement = anElement;
  }
  
  G4bool ParticleHPChannel::Register(ParticleHPFinalState *theFS)
  {
	registerCount++;
...
*/
    ...
    theIsotopeWiseData = new ParticleHPIsoData [niso];
    delete [] active;
    active = new G4bool[niso];

    delete [] theFinalStates;
    theFinalStates = new ParticleHPFinalState * [niso];
  ...
  //void ParticleHPChannel::UpdateData(G4int A, G4int Z, G4int index, G4double abundance)
  void ParticleHPChannel::UpdateData(G4int A, G4int Z, G4int M, G4int index, G4double abundance, G4ParticleDefinition* projectile)
  {
  ...
  void ParticleHPChannel::Harmonise(ParticleHPVector *& theStore, ParticleHPVector * theNew)
  {
    G4int s_tmp = 0, n=0, m_tmp=0;
    ParticleHPVector * theMerge = new ParticleHPVector;
    ParticleHPVector * anActive = theStore;
    ParticleHPVector * aPassive = theNew;
...
#include "ParticleHPThermalBoost.hh"
  ...
  G4HadFinalState * ParticleHPChannel::
  ApplyYourself(const G4HadProjectile & theTrack, G4int anIsotope)
  {
//    G4cout << "ParticleHPChannel::ApplyYourself+"<<niso<<G4endl;
    if ( anIsotope != -1 && anIsotope != -2 ) 
    {
       //Inelastic Case
       //G4cout << "ParticleHPChannel Inelastic Case" 
       //<< " Z= " << this->GetZ(it) << " A = " << this->GetN(it) << G4endl;
       ParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargA( (G4int)this->GetN(anIsotope) ); 
       ParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargZ( (G4int)this->GetZ(anIsotope) ); 
    ...
    if(sum == 0) 
    {
//      G4cout << "ParticleHPChannel::ApplyYourself theFinalState->Initialize+"<<G4endl;
//      G4cout << "ParticleHPChannel::ApplyYourself theFinalState->Initialize-"<<G4endl;
...
//      G4cout << "TESTHP 23 NISO="<<niso<<G4endl;
      ...
      G4double running=0;
//      G4cout << "ParticleHPChannel::ApplyYourself Done the sum"<<niso<<G4endl;
...
//	      G4cout << "TESTHP 24 it="<<it<<G4endl;
    ...
    //G4cout << "TK ParticleHPChannel Elastic, Capture and Fission Cases "
    //<< " Z= " << this->GetZ(it) << " A = " << this->GetN(it) << G4endl;
    ParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargA( A );
    ParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargZ( Z );
    ParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargM( M );

    return theFinalState;
  }


void ParticleHPChannel::DumpInfo(){
