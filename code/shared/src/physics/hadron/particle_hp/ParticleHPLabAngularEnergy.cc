...
void ParticleHPLabAngularEnergy::Init(std::istream & aDataFile)
{
  aDataFile >> nEnergies;
  theManager.Init(aDataFile);
  theEnergies = new G4double[nEnergies];
  nCosTh = new G4int[nEnergies];
  theData = new ParticleHPVector * [nEnergies];
  ...
  for(G4int i=0; i<nEnergies; i++)
  {
    ...
    theData[i] = new ParticleHPVector[nCosTh[i]];
    G4double label;
...
G4ReactionProduct * ParticleHPLabAngularEnergy::Sample(G4double anEnergy, G4double massCode, G4double )
{
   ...
   {
     throw G4HadronicException(__FILE__, __LINE__, "ParticleHPLabAngularEnergy: Unknown ion case 2");
   }
   ...
   else // this is the small big else.
   {
     ...
     x1 = theEnergies[it-1];
     x2 = theEnergies[it];
     ParticleHPVector thBuff1; // to be interpolated as run1.
