...
#include "G4PhysicsModelCatalog.hh"
 ...
 ParticleHPFissionFS::ParticleHPFissionFS()
 {
 ...
 void ParticleHPFissionFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String & aFSType, G4ParticleDefinition* projectile )
 {
    //G4cout << "ParticleHPFissionFS::Init " << A << " " << Z << " " << M << G4endl;
    ...
    if ( ParticleHPManager::GetInstance()->GetProduceFissionFragments() && theFF.HasFSData() ) 
    {
 ...
 G4HadFinalState * ParticleHPFissionFS::ApplyYourself(const G4HadProjectile & theTrack)
 {  

    //Because it may change by UI command 
    produceFissionFragments=ParticleHPManager::GetInstance()->GetProduceFissionFragments(); 

 //G4cout << "ParticleHPFissionFS::ApplyYourself " << G4endl;
// prepare neutron
...
//G4cout << "delayed" << G4endl;
     for(i0=Prompt; i0<Prompt+delayed; i0++)
     {
       ...
       if ( theDecayConstants[i0-Prompt] > 1.0e-30 ) {
         ...
         G4Exception( "ParticleHPFissionFS::ApplyYourself ", "HAD_FISSIONHP_001", JustWarning, ed );
       }
...
//    G4cout <<"Number of delayed = "<<delayed<<G4endl;
   ...
   ParticleHPFissionERelease * theERelease = theFS.GetEnergyRelease();
