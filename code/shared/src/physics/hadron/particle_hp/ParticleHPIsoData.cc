...
#include "G4IonTable.hh"
  ...
  //G4bool ParticleHPIsoData::Init(G4int A, G4int Z, G4double abun, G4String dirName, G4String aFSType)
G4bool ParticleHPIsoData::Init(G4int A, G4int Z, G4int M, G4double abun, G4String dirName, G4String aFSType)
  {
    theChannelData = 0;
    G4double abundance = abun/100.;
    G4String filename;
    G4bool result = true;
    //ParticleHPDataUsed aFile = theNames.GetName(A, Z, dirName, aFSType, result);
    ParticleHPDataUsed aFile = theNames.GetName(A, Z, M, dirName, aFSType, result);
    filename = aFile.GetName();
//    if(filename=="") return false;
    //std::ifstream theChannel(filename);
    std::istringstream theChannel(filename,std::ios::in);
    ParticleHPManager::GetInstance()->GetDataStream(filename,theChannel);

#ifdef G4PHPDEBUG
    if(std::getenv("ParticleHPDebug")) G4cout << "ParticleHPIsoData::Init = "<< filename <<" "<< A << " " << Z <<G4endl;
#endif
    
    if(Z==1 && (aFile.GetZ()!=Z || std::abs(aFile.GetA()-A)>0.0001) )
    {
      if(std::getenv("ParticleHPDebug")) G4cout << "Skipped = "<< filename <<" "<<A<<" "<<Z<<G4endl;
      //080901 TKDB No more necessary below protection, cross sections set to 0 in ParticleHPNames
...
//    theChannel.close();
  ...
  //void ParticleHPIsoData::Init(G4int A, G4int Z, G4double abun) //fill PhysicsVector for this Isotope
#include "G4Proton.hh"
#include "G4Deuteron.hh"
#include "G4Triton.hh"
#include "G4He3.hh"
#include "G4Alpha.hh"
void ParticleHPIsoData::Init(G4int A, G4int Z, G4int M,G4double abun, G4ParticleDefinition* projectile, const char* dataDirVariable ) //fill PhysicsVector for this Isotope
  {
...
//  if(theFissionData!=0) theFissionData->Dump();
  ...
  G4String ParticleHPIsoData::GetName(G4int A, G4int Z, G4String base, G4String rest)
  {
    G4bool dbool;
