...
ENDFTapeRead::
ENDFTapeRead( G4String FileLocation,
                G4String FileName,
...
ENDFTapeRead::
ENDFTapeRead( std::istringstream& dataStream,
                G4FFGEnumerations::YieldType WhichYield,
                G4FFGEnumerations::FissionCause /*WhichCause*/,
                G4int Verbosity )
:   /*Cause_(WhichCause),*/
...
Initialize( G4String dataFile )
{
    ...
    ParticleHPManager::GetInstance()->GetDataStream(dataFile, dataStream);
...
G4ENDFYieldDataContainer* ENDFTapeRead::
...
G4FFG_FUNCTIONENTER__
    ...
    if(!dataStream.good())
    {
        G4Exception("ENDFTapeRead::ReadInData()",
                    "Illegal file name",
                    JustWarning,
