...
ParticleHPCaptureData::ParticleHPCaptureData()
...
G4bool ParticleHPCaptureData::IsIsoApplicable( const G4DynamicParticle* dp , 
                                                G4int /*Z*/ , G4int /*A*/ ,
                                                const G4Element* /*elm*/ ,
...
G4double ParticleHPCaptureData::GetIsoCrossSection( const G4DynamicParticle* dp ,
                                   G4int /*Z*/ , G4int /*A*/ ,
                                   const G4Isotope* /*iso*/  ,
...
/*
G4bool ParticleHPCaptureData::IsApplicable(const G4DynamicParticle*aP, const G4Element*)
{
  G4bool result = true;
...
void ParticleHPCaptureData::BuildPhysicsTable(const G4ParticleDefinition& aP)
{
  if(&aP!=G4Neutron::Neutron()) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");  

   if ( G4Threading::IsWorkerThread() ) {
      theCrossSections = ParticleHPManager::GetInstance()->GetCaptureCrossSections();
      return;
   }
  
  size_t numberOfElements = G4Element::GetNumberOfElements();
  // G4cout << "CALLED ParticleHPCaptureData::BuildPhysicsTable "<<numberOfElements<<G4endl;
   // TKDB
  ...
  for( size_t i=0; i<numberOfElements; ++i )
  {
     ...
     #endif
     G4PhysicsVector* physVec = ParticleHPData::
      Instance(G4Neutron::Neutron())->MakePhysicsVector((*theElementTable)[i], this);
     theCrossSections->push_back(physVec);
  }

  ParticleHPManager::GetInstance()->RegisterCaptureCrossSections( theCrossSections );
}

void ParticleHPCaptureData::DumpPhysicsTable(const G4ParticleDefinition& aP)
{
  if(&aP!=G4Neutron::Neutron()) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");
...
//
   ...
   //G4cout << "ParticleHPCaptureData::DumpPhysicsTable still to be implemented"<<G4endl;
   #endif
}

#include "G4NucleiProperties.hh"

G4double ParticleHPCaptureData::
...
G4int ParticleHPCaptureData::GetVerboseLevel() const 
{
   return ParticleHPManager::GetInstance()->GetVerboseLevel();
}
void ParticleHPCaptureData::SetVerboseLevel( G4int newValue ) 
{
   ParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}
void ParticleHPCaptureData::CrossSectionDescription(std::ostream& outFile) const
{
