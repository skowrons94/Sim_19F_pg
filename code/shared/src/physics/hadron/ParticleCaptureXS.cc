...
G4ElementData* ParticleCaptureXS::data[] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
G4String ParticleCaptureXS::gDataDirectory[] = {"", "", "", "", "", ""};

#ifdef G4MULTITHREADED
  G4Mutex ParticleCaptureXS::particleCaptureXSMutex = G4MUTEX_INITIALIZER;
#endif

ParticleCaptureXS::ParticleCaptureXS(const G4ParticleDefinition* part) 
  : G4VCrossSectionDataSet("ParticleCaptureXS"),
    particle(part)
{
  if(nullptr == part) {
    G4Exception("ParticleCaptureXS::ParticleCaptureXS(..)","had015",
		FatalException, "NO particle definition in constructor");
  } else {
    verboseLevel = 1;
    const G4String& particleName = particle->GetParticleName();
    if(verboseLevel > 1) {
      G4cout << "ParticleCaptureXS::ParticleCaptureXS for " 
	...
	G4Exception("ParticleCaptureXS::BuildPhysicsTable(..)","had012",
		    FatalException, ed, "");
      } 
    }
  }
...
void ParticleCaptureXS::CrossSectionDescription(std::ostream& outFile) const
{
  outFile << "ParticleCaptureXS calculates n, p, d, t, he3, he4 capture\n"
...
G4bool 
ParticleCaptureXS::IsElementApplicable(const G4DynamicParticle*, 
...
G4bool 
ParticleCaptureXS::IsIsoApplicable(const G4DynamicParticle*,
				       G4int, G4int,
...
G4double ParticleCaptureXS::GetElementCrossSection(
	 ...
	 G4int ZZ, const G4Material*)
{
  ...
  //  G4cout  << "ParticleCaptureXS::GetCrossSection e= " << ekin 
  //  << " Z= " << Z << G4endl;
...
G4double ParticleCaptureXS::GetIsoCrossSection(
         const G4DynamicParticle* aParticle, 
	 G4int Z, G4int A,
...
G4double 
ParticleCaptureXS::IsoCrossSection(G4double ekin, G4double logE,
                                       G4int ZZ, G4int A)
{
  G4double xs = 0.0;
  G4int Z = (ZZ >= MAXZCAPTURE) ? MAXZCAPTURE - 1 : ZZ; 
  ...
  /*
  G4cout << "ParticleCaptureXS: IsoCrossSection  Z= " 
         << Z << "  A= " << A 
         << "  Amin= " << amin[Z] << " Amax= " << amax[Z]
         << " E(MeV)= " << ekin << " Emax: " << pv->GetMaxEnergy() << G4endl;
  */
  ...
  if(ekin <= emax && amin[Z] < amax[Z] && A >= amin[Z] && A <= amax[Z]) {
    ...
    if(pviso != nullptr) { 
      ...
      if(verboseLevel > 1) {
	G4cout << "ParticleCaptureXS::IsoXS: for " 
...
const G4Isotope* ParticleCaptureXS::SelectIsotope(
...
void 
ParticleCaptureXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  //G4cout << "ParticleCaptureXS particle name: " << p.GetParticleName() << "  ID: " << p.GetParticleDefinitionID() << G4endl;
  if(verboseLevel > 0){
    G4cout << "ParticleCaptureXS::BuildPhysicsTable for " 
...
#ifdef G4MULTITHREADED
    G4MUTEXLOCK(&particleCaptureXSMutex);
    if(data[index] == nullptr) { 
#endif
      isMaster = true;
      data[index] = new G4ElementData();
      data[index]->SetName(particle->GetParticleName() + "Capture");
      FindDirectoryPath();
#ifdef G4MULTITHREADED
    }
    G4MUTEXUNLOCK(&particleCaptureXSMutex);
#endif
  }
...
const G4String& ParticleCaptureXS::FindDirectoryPath()
{
  ...
  if(gDataDirectory[index].empty()) {
    ...
    if (nullptr != path) {
      ...
      ost << path << "/" << particle->GetParticleName() << "/cap";
      gDataDirectory[index] = ost.str();
    } else {
      G4Exception("G4ParticleCaptureXS::Initialise(..)","had013",
		  FatalException,
...
void ParticleCaptureXS::InitialiseOnFly(G4int Z)
{
#ifdef G4MULTITHREADED
   G4MUTEXLOCK(&particleCaptureXSMutex);
...
void ParticleCaptureXS::Initialise(G4int Z)
{
  ...
  /*
  G4cout << "ParticleCaptureXS::Initialise for Z= " << Z 
	 << " idx= " << index
	 << "  Amin= " << amin[Z] 
	 << "  Amax= " << amax[Z] 
  ...
  */
  ...
  /*
  G4cout << "ParticleCaptureXS: index= " << index 
         << " Z= " << Z << " coeff= " << coeff[Z][index] << G4endl; 
  */
}

G4PhysicsVector* 
ParticleCaptureXS::RetrieveVector(std::ostringstream& ost, G4bool warn)
{
  ...
  if (!filein.is_open()) {
    if(warn) { 
	 ...
	 << "> is not opened!";
      G4Exception("ParticleCaptureXS::RetrieveVector(..)","had014",
		  FatalException, ed, "Check G4PARTICLEXSDATA");
    }
  } else {
    if(verboseLevel > 1) {
      G4cout << "File " << ost.str() 
	     << " is opened by ParticleCaptureXS" << G4endl;
    }
    ...
    if(!v->Retrieve(filein, true)) {
      ...
      ed << "Data file <" << ost.str().c_str()
	 << "> is not retrieved!";
      G4Exception("ParticleCaptureXS::RetrieveVector(..)","had015",
		  FatalException, ed, "Check G4PARTICLEXSDATA");
    }
  }
  return v;
