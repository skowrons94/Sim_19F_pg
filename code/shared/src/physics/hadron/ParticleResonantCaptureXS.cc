...
G4ElementData* ParticleResonantCaptureXS::data[] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
G4String ParticleResonantCaptureXS::gDataDirectory[] = {"", "", "", "", "", ""};

#ifdef G4MULTITHREADED
  G4Mutex ParticleResonantCaptureXS::particleResonantCaptureXSMutex = G4MUTEX_INITIALIZER;
#endif

ParticleResonantCaptureXS::ParticleResonantCaptureXS(const G4ParticleDefinition* part) 
  : G4VCrossSectionDataSet("ParticleResonantCaptureXS"),
    particle(part)
{
  if(nullptr == part) {
    G4Exception("ParticleResonantCaptureXS::ParticleResonantCaptureXS(..)","had015",
		FatalException, "NO particle definition in constructor");
  } else {
    verboseLevel = 1;
    const G4String& particleName = particle->GetParticleName();
    if(verboseLevel > 1) {
      G4cout << "ParticleResonantCaptureXS::ParticleResonantCaptureXS for " 
	     << particleName << " on atoms with Z < " << MAXZCAPTURE << G4endl;
    }
	...
	G4Exception("ParticleResonantCaptureXS::BuildPhysicsTable(..)","had012",
		    FatalException, ed, "");
      } 
    }
  }
...
void ParticleResonantCaptureXS::CrossSectionDescription(std::ostream& outFile) const
{
  outFile << "ParticleResonantCaptureXS calculates n, p, d, t, he3, he4 capture\n"
...
G4bool 
ParticleResonantCaptureXS::IsElementApplicable(const G4DynamicParticle*, 
...
G4bool 
ParticleResonantCaptureXS::IsIsoApplicable(const G4DynamicParticle*,
				       G4int, G4int,
...
G4double ParticleResonantCaptureXS::GetElementCrossSection(
	 ...
	 G4int ZZ, const G4Material*)
{
  ...
  G4int Z = (ZZ >= MAXZCAPTURE) ? MAXZCAPTURE - 1 : ZZ; 

  auto pv = GetPhysicsVector(Z);
  if(nullptr == pv) { return xs; }
  //  G4cout  << "ParticleResonantCaptureXS::GetCrossSection e= " << ekin 
  //  << " Z= " << Z << G4endl;
...
G4double ParticleResonantCaptureXS::GetIsoCrossSection(
         const G4DynamicParticle* aParticle, 
	 G4int Z, G4int A,
...
G4double 
ParticleResonantCaptureXS::IsoCrossSection(G4double ekin, G4double logE,
                                       G4int ZZ, G4int A)
{
  G4double xs = 0.0;
  G4int Z = (ZZ >= MAXZCAPTURE) ? MAXZCAPTURE - 1 : ZZ; 
  /*  
  G4cout << "ParticleResonantCaptureXS: IsoCrossSection  Z= " 
         << Z << "  A= " << A 
  ...
  */
  ...
  if(ekin <= emax && amin[Z] < amax[Z] && A >= amin[Z] && A <= amax[Z]) {
    ...
    if(pviso != nullptr) { 
      ...
      if(verboseLevel > 1) {
	G4cout << "ParticleResonantCaptureXS::IsoXS: for " 
...
const G4Isotope* ParticleResonantCaptureXS::SelectIsotope(
...
void 
ParticleResonantCaptureXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  //G4cout << "ParticleResonantCaptureXS particle name: " << p.GetParticleName() << "  ID: " << p.GetParticleDefinitionID() << G4endl;
  if(verboseLevel > 0){
    G4cout << "ParticleResonantCaptureXS::BuildPhysicsTable for " 
...
#ifdef G4MULTITHREADED
    G4MUTEXLOCK(&particleResonantCaptureXSMutex);
    if(data[index] == nullptr) { 
#endif
      isMaster = true;
      data[index] = new G4ElementData();
      data[index]->SetName(particle->GetParticleName() + "ResonantCapture");
      FindDirectoryPath();
#ifdef G4MULTITHREADED
    }
    G4MUTEXUNLOCK(&particleResonantCaptureXSMutex);
#endif
  }
...
const G4String& ParticleResonantCaptureXS::FindDirectoryPath()
{
  ...
  if(gDataDirectory[index].empty()) {
    ...
    if (nullptr != path) {
      ...
      ost << path << "/" << particle->GetParticleName() << "/rescap";
      gDataDirectory[index] = ost.str();
    } else {
      G4Exception("G4ParticleResonantCaptureXS::Initialise(..)","had013",
		  FatalException,
...
void ParticleResonantCaptureXS::InitialiseOnFly(G4int Z)
{
#ifdef G4MULTITHREADED
   G4MUTEXLOCK(&particleResonantCaptureXSMutex);
...
void ParticleResonantCaptureXS::Initialise(G4int Z)
{
  ...
  /*
  G4cout << "ParticleResonantCaptureXS::Initialise for Z= " << Z 
	 << " idx= " << index
	 << "  Amin= " << amin[Z] 
	 << "  Amax= " << amax[Z] 
  ...
  */
  ...
  /*
  G4cout << "ParticleResonantCaptureXS: index= " << index 
         << " Z= " << Z << " coeff= " << coeff[Z][index] << G4endl; 
  */
}

G4PhysicsVector* 
ParticleResonantCaptureXS::RetrieveVector(std::ostringstream& ost, G4bool warn)
{
  ...
  if (!filein.is_open()) {
    if(warn) { 
	 ...
	 << "> is not opened!";
      G4Exception("ParticleResonantCaptureXS::RetrieveVector(..)","had014",
		  FatalException, ed, "Check G4PARTICLEXSDATA");
    }
  } else {
    if(verboseLevel > 1) {
      G4cout << "File " << ost.str() 
	     << " is opened by ParticleResonantCaptureXS" << G4endl;
    }
    ...
    if(!v->Retrieve(filein, true)) {
      ...
      ed << "Data file <" << ost.str().c_str()
	 << "> is not retrieved!";
      G4Exception("ParticleResonantCaptureXS::RetrieveVector(..)","had015",
		  FatalException, ed, "Check G4PARTICLEXSDATA");
    }
  }
  return v;
