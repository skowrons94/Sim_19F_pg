...
//
#ifndef ParticleHPProduct_h
#define ParticleHPProduct_h 1
...
#include "ParticleHPDiscreteTwoBody.hh"
#include "ParticleHPIsotropic.hh"
#include "ParticleHPNBodyPhaseSpace.hh"
#include "ParticleHPLabAngularEnergy.hh"
#include "G4Cache.hh"
class G4ParticleDefinition;

enum G4HPMultiMethod { G4HPMultiPoisson, G4HPMultiBetweenInts };

class ParticleHPProduct
{
   struct toBeCached
   {
...
public:
  ...
  ParticleHPProduct()
  {
	...
	throw G4HadronicException(__FILE__, __LINE__, ("multiplicity method unknown to ParticleHPProduct" + G4String(method)).c_str());
      }
    }
    else
    {
  ...
  inline void Init(std::istream & aDataFile, G4ParticleDefinition* projectile)
  {
    ...
    if( std::getenv("G4PHPTEST") )
      G4cout << " ParticleHPProduct :: Init MassCode "
             << theMassCode << " " << theMass << " theActualStateQValue "
             << theActualStateQValue << G4endl;// GDEB
    if( std::getenv("G4PHPTEST") )
      G4cout << " ParticleHPProduct :: Init theActualStateQValue "
             << theActualStateQValue << G4endl;// GDEB
    ...
    if(theDistLaw==0)
    {
      ...
      theDist = new ParticleHPIsotropic;
    }
    else if(theDistLaw == 1)
    {
      // Continuum energy-angular distribution
      theDist = new ParticleHPContEnergyAngular(projectile);
    }
    else if(theDistLaw == 2)
    {
      // Discrete 2-body scattering
      theDist = new ParticleHPDiscreteTwoBody;
    }
    ...
    {
      // N-Body phase space
      theDist = new ParticleHPNBodyPhaseSpace;
    }
    else if(theDistLaw == 7)
    {
      // Laboratory angular energy paraetrisation
      theDist = new ParticleHPLabAngularEnergy;
    }
    else
    {
      throw G4HadronicException(__FILE__, __LINE__, "distribution law unknown to ParticleHPProduct");
    }
    if(theDist!=0)
    {
