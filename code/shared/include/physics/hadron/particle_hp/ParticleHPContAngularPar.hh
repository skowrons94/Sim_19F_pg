...
//
#ifndef ParticleHPContAngularPar_h
#define ParticleHPContAngularPar_h 1
...
class ParticleHPContAngularPar
{
  ...
  ParticleHPContAngularPar(G4ParticleDefinition* projectile); 
  ...
  { 
    if( std::getenv("G4PHPTEST") )
      G4cout << this << " ParticleHPContAngularPar::GetEnergy "
  ...
  void BuildByInterpolation(G4double anEnergy, G4InterpolationScheme aScheme, 
             ParticleHPContAngularPar & store1, 
             ParticleHPContAngularPar & store2);
    // NOTE: this interpolates legendre coefficients

  void PrepareTableInterpolation(const ParticleHPContAngularPar* angularPrev);
  ...
  G4double MeanEnergyOfThisInteraction()
  {
    ...
    {
      return 0;
      // throw G4HadronicException(__FILE__, __LINE__, "ParticleHPContAngularPar: Logical error in Product class");
    }
    else
    {
  ...
  ParticleHPList* GetAngDataList() const
  {
    return theAngular; 
  }
