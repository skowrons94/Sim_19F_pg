...
//
#ifndef ParticleHPVector_h
#define ParticleHPVector_h 1
...
{
  friend ParticleHPVector & operator + (ParticleHPVector & left, 
                                         ParticleHPVector & right);
  
  public:
  
  ParticleHPVector();

  ParticleHPVector(G4int n);
  
  ~ParticleHPVector();
  
  ParticleHPVector & operator = (const ParticleHPVector & right);
  ...
  inline void SetPoint(G4int i, const ParticleHPDataPoint & it)
  {
    G4double x = it.GetX();
    G4double y = it.GetY();
    SetData(i, x, y);
  ...
  { 
//    G4cout <<"ParticleHPVector::SetData called"<<nPoints<<" "<<nEntries<<G4endl;
    Check(i);
  ...
  inline const ParticleHPDataPoint & GetPoint(G4int i) const { return theData[i]; }
  ...
  void Init(std::istream & aDataFile,G4double ux=1., G4double uy=1.)
  {
    ...
    theData = new ParticleHPDataPoint[total]; 
    nPoints=total;
    nEntries=0;    
    theManager.Init(aDataFile);
...
//080811 TK DB 
  ...
  inline void Merge(ParticleHPVector * active, ParticleHPVector * passive)
  {
    CleanUp();
  ...
  inline void Integrate() 
  {
    ...
    for(i=1;i<GetVectorLength();i++)
    {
      if(std::abs((theData[i].GetX()-theData[i-1].GetX())/theData[i].GetX())>0.0000001)
      {
        ...
        {
          throw G4HadronicException(__FILE__, __LINE__, "Unknown interpolation scheme in ParticleHPVector::Integrate");
        }
...
//  void SetBuffered(const std::vector<G4double> &aBuffer) {theBuffered = aBuffer;}
  ...
  ParticleHPInterpolator theLin;
  
  private:
  
  G4double totalIntegral;
  
  ParticleHPDataPoint * theData; // the data
